...
export default function ContentionPage() {
  ...
  const delayedCount = rows.filter(
    (r) =>
      r.soloIndex !== null && r.togetherIndex !== null && r.togetherIndex > r.soloIndex,
  ).length;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {delayedCount > 0 ? (
        <Card className="border-warning/40 bg-warning-soft text-warning p-4 text-sm">
          {delayedCount} of {rows.length} goal{delayedCount === 1 ? "" : "s"} would be
          funded sooner if it were the only one you were saving for. That is the cost of
          running them together.
        </Card>
      ) : (
        <Card className="border-success/40 bg-success-soft text-success p-4 text-sm">
          Every goal lands on the same month whether it competes for your surplus or
          not. There is no real contention right now.
        </Card>
      )}
